...
def get_accesstoken(config):
    ...
    """
    Client Credentials Grant認証でアクセストークンを取得する
    ...
    if __CONFIG_KEY_CLIENT_SECRET not in config:
        ...
        logger.debug(config)
        raiseException("invalid config setting.")

    try:

        # form dataを作成
        # ----------------------------------------------------
        post_form_data = urllib.parse.urlencode({'grant_type': 'client_credentials'}).encode('utf-8')
